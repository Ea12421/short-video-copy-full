...
version: 0.6.2-candidate
language: zh-CN
---

# Short-video copy full candidate v0.6.2
...
使用项目内 `references/release-analysis-value-filter.md`。模型身份、当前表现、服务路由和长期路线必须分开；核心信息段要新增任务差异、机制差异、可决策比较、后果或复核动作，纯过渡段不重复播报属性。API、多模态、兼容和代码支持只有具体改变任务或调用选择时才进入正文，否则放入来源卡。只有跨模型比较被锁为本稿角度时，才写清对象、任务、条件、结果和局限。

用户要求架构、硬件、推理或成本机制，或公开材料确实足够时，可启用 `references/technical-deep-dive-route.md`。把公开硬事实、第三方观察和作者推测分层，沿“机制→瓶颈→系统后果→成本/产品影响→可验证判断”写一条因果链；没有机制证据时不硬凑技术深度。
...
开始角度地图前，按 `references/brief-clarification-route.md` 先读现有材料；遇到时效、技术或事实会改变选项时，先做 1–3 个关键问题的有界上下文探测，再生成 A/B/C 三个预设方向和 D「自定义」。A 为基于证据的推荐项，并说明每个方向的取舍。只问一轮、最多一个核心问题；允许混选，其他低影响字段使用可见假设。用户不回答时采用 A 继续，不把问题清单念进稿子。
...
不要先替观众宣布“最有意思”“更值得看”“影响更大”；直接给出会改变判断的事实、对照或任务后果。后句没有新增证据时，删掉价值宣告句。
